...
3.2.3.	信息展示模块概要设计	8
3.2.4.	后台管理模块概要设计	8
3.2.5.	web服务端处理请求模块概要设计	9
3.2.6.	模块功能关系	9
3.3.	详细设计	10
3.3.1.	用户交互模块详细设计	10
3.3.2.	信息展示模块详细设计	14
3.3.3.	后台管理模块详细设计	16
3.3.4.	web服务端处理请求模块详细设计	17
3.4.	本章小结	19
4.	测试与验证	20
4.1.	测试效果	20
4.1.1.	首页运行效果	20
...
4.3.	用户交互模块测试	24
4.3.1.	注册登录功能测试用例	24
4.3.2.	评论收藏购买功能测试用例	25
4.3.3.	信息修改功能测试用例	25
4.3.4.	商品搜索功能测试用例	26
4.4.	后台管理模块测试	26
4.4.1.	数据爬取功能测试用例	26
4.4.2.	用户管理功能测试用例	27
4.4.3.	管理员管理功能测试用例	27
4.5.	WEB服务端处理请求模块测试	28
4.5.1.	搜索请求处理测试用例	28
4.5.2.	注册登录请求处理测试用例	28
4.5.3.	评论或收藏请求处理测试用例	29
4.5.4.	数据爬取请求处理测试用例	30
4.6.	本章小结	30
5.	评价	31
5.1.	对项目开发过程的体会	31
5.1.1.	过程总结	31
...
3.2.	用户交互模块	69
3.2.1.	注册登录功能测试用例	69
3.2.2.	评论收藏购买功能测试用例	70
3.2.3.	信息修改功能测试用例	71
3.2.4.	商品搜索功能测试用例	71
3.3.	后台管理模块	72
3.3.1.	数据爬取功能测试用例	72
3.3.2.	用户管理功能测试用例	72
3.3.3.	管理员管理功能测试用例	73
3.4.	WEB服务端处理请求模块	73
3.4.1.	搜索请求处理测试用例	73
3.4.2.	注册登录请求处理测试用例	74
3.4.3.	评论或收藏请求处理测试用例	75
3.4.4.	数据爬取请求处理测试用例	75
4.	测试结果分析	76
附录4 用户文档说明书	77
1.	网站描述	77
2.	使用说明	77
...
但是，网站的庞大意味着网站信息的复杂，这也导致用户在选择商品时越来越困难，甚至有很多杂乱的广告严重影响用户体验。而人们购物的选择则是基于物品的外观、价格和评价等，其中，商品的评论数据很大程度上影响着用户购买的选择。用户进入购物网站，看到商品，会浏览关于商品的评论，帮助自己分析，最后决定是否购买。但是，随着网络站点的大量增长，网络中的信息量逐渐增大，非常庞杂的信息使得用户在挑选自己满意的商品时显得非常麻烦，海量的商品信息让用户在选择时非常困难。
...
2.4.	Express请求机制简介
Express是一个搭建极简、灵活的 web 应用的开发框架，在处理请求方面也有一套机制。客户端发起请求，然后web服务端进行处理，服务端返回相关数据结果给客户端。客户端要发起请求，首先需要一个标识，在Web应用中通常情况下是URL，通过这个标识将请求发送给web服务端的某个具体处理程序，在这个过程中，请求可能会经历一系列全局处理，比如验证、授权、URL解析等，然后才定位到某个控制处理程序进行业务处理，最后将生成的数据返回给客户端，客户端将数据结合视图模版呈现在用户面前。而从客户端请求到web服务端端处理程序的过程，也就叫做路由，其实就是如何定位到web服务端处理程序的过程。Express正是有这样的机制，通过解析客户端传递过来的URL，对URL进行解析，并分发至不同的控制处理程序，最终程序处理完的数据返回给客户端。
2.5.	Superagent简介
...
3.2.	概要设计
3.2.1.	系统结构设计
本系统包括客户端和web服务端。客户端主要划分成3个模块：用户交互模块、信息展示模块和后台管理模块。web服务端主要用于处理客户端的请求，统一为一个模块：web服务端处理请求模块。每个模块包含了相应的功能点，整个系统的结构如图3.1.1，网络模型如图3.1.2：
...
4)	逻辑流程
	搜索请求处理逻辑流程：
web服务端解析出查询的关键词并对之进行处理，调用数据库访问接口，传入sql语句进行查询，然后传回封装好的查询结果，最后回传到客户端。流程如图3.20：
 
图3.20搜索请求处理逻辑流程图
	数据请求处理逻辑流程：
解析接收到的请求信息，连接数据库查询，将结果进行封装后返回给客户端。如图3.21：
 
图3.21数据请求处理逻辑流程图
	请求路由分发逻辑流程：
分发不同请求至不同的控制器进行处理，并对处理的结果返回至相应的客户端。流程如图3.22：
...
图4.1 首页运行效果图
4.1.2.	评论蛛网图表运行效果
图4.2为客户端部分评论蛛网图表运行效果图，其中好评率、正品、质量不错、没有色差、穿着舒适、物流快等流向数据是对爬取到的数据进行分析，最终以可视化的图表进行展示：
 
图4.2 客户端部分评论蛛网图表运行效果图
4.1.3.	评论柱状图表运行效果
图4.3为客户端部分评论柱状图表运行效果图，其中好评率、正品、质量不错、没有色差、穿着舒适、物流快等流向数据是对爬取到的数据进行分析，灰色代表标准分，鲜色代表实际分。最终以可视化的图表进行展示：
...
4.1.	客户端行为模型
4.1.1.	登录行为模型
登录行为主要是用户进入登陆页，输入符合规范的信息，点击登录，后台处理登录请求返回结果给客户端，继续等待用户操作，图4.1是对登录模块进行状态描述。
 
图4.1登录行为状态图
4.1.2.	注册行为模型
注册行为主要是用户进入注册页，输入符合规范的信息，点击注册，后台处理注册请求返回结果给客户端，继续等待用户操作，图4.2是对注册模块进行状态描述。
 
图4.2注册行为状态图
4.1.3.	评论行为模型
评论行为主要是用户进入品牌评论页，输入符合规范的评论内容，点击发表评论，后台处理请求返回结果给客户端，继续等待用户操作，图4.3是对评论模块进行状态描述。
 
图4.3评论行为状态图
4.1.4.	收藏行为模型
收藏行为主要是用户进入商品详情页，登录状态下点击收藏，后台处理收藏请求返回结果给客户端，继续等待用户操作，图4.4是对收藏模块进行状态描述。
 
图4.4收藏行为状态图
4.1.5.	信息修改行为模型
信息修改行为主要是用户进入信息修改页，输入符合规范的原始密码新密码，点击修改，后台处理修改请求返回结果给客户端，继续等待用户操作，图4.5是对信息修改模块进行状态描述。
 
图4.5信息修改行为状态图
4.1.6.	用户管理行为模型
用户管理行为主要是管理员进入商品详情页，点击功能按钮触发请求，后台处理请求返回结果给客户端，继续等待管理员操作，图4.6是对用户管理模块进行状态描述。
 
图4.6用户管理行为状态图
4.1.7.	数据爬取行为模型
数据爬取行为主要是管理员进入数据爬取页，点击功能按钮触发数据爬取请求，后台处理请求爬取数据将数据存入数据库并返回结果给客户端，继续等待用户操作，图4.7是对数据爬取模块进行状态描述。
 
图4.7数据爬取行为状态图
4.2.	服务端行为模型
4.2.1.	搜索请求处理行为模型
搜索请求处理行为主要是服务端接收客户端搜索请求，解析搜索关键词并对数据库进行查询，返回结果给客户端，继续等待搜索请求，图4.8是搜索请求处理模块进行状态描述。
 
图4.8搜索请求处理行为状态图
4.2.2.	数据传输请求行为模型
数据传输请求处理行为主要是服务端接收客户端数据传输请求，解析请求方法并对数据库进行增删查改，返回结果给客户端，继续等待新的请求，图4.9是数据传输请求处理模块进行状态描述。
 
图4.9数据传输请求处理行为状态图
4.2.3.	数据爬取请求处理行为模型
数据爬取请求处理行为主要是服务端接收客户端数据爬取请求，解析需要请求的URL并开始进行数据爬取，将爬取到的数据存入数据库，返回结果给客户端，继续等待新的请求，图4.10是数据爬取请求处理模块进行状态描述。
 
图4.10数据爬取请求处理行为状态图
4.2.4.	请求路由分发行为模型
请求路由分发行为主要是服务端接收客户端请求，解析请求并将请求分发至不同的控制器，控制器控制数据，返回结果给客户端，继续等待新的请求，图4.11是请求路由分发模块进行状态描述。
...
3.5.5.	流程逻辑
	搜索请求处理逻辑流程：
web服务端解析出查询的关键词并对之进行处理，调用数据库访问接口，传入sql语句进行查询，然后传回封装好的查询结果，最后回传到客户端。流程如图3.14：
 
图3.14搜索请求处理逻辑流程图
	数据请求处理逻辑流程：
解析接收到的请求信息，连接数据库查询，将结果进行封装后返回给客户端。如图3.15：
