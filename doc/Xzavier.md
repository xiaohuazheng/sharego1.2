...
为了解决用户网上购物时湮没在海量的信息海洋中而不能选择到有用的信息的问题，为了帮助用户对商品的网络口碑、用户评价有一个快速、直观的理解，本网站利用网络爬虫爬取目标网站的商品信息，并对商品信息分类展示，爬取商品的主要评价数据，对评论数据进行图表化，提供给用户直观的可视化评论分析图，突出商品商品优缺点，以帮助用户进行购物选择，帮助用户区分商品是否值得购买，节约时间成本，购买到适合的产品。
...
4.3.用户交互模块测试	30
4.3.1.	注册登录功能测试用例	30
4.3.2.	评论收藏购买功能测试用例	31
4.3.3.	信息修改功能测试用例	32
4.3.4.	商品搜索功能测试用例	33
4.4.后台管理模块测试	33
4.4.1.	商品管理能测试用例	33
4.4.2.	用户管理功能测试用例	33
4.5. WEB服务端处理请求模块测试	34
4.5.1.	搜索请求处理测试用例	34
4.5.2.	注册登录请求处理测试用例	35
4.5.3.	评论或收藏请求处理测试用例	35
4.5.4.	数据爬取请求处理测试用例	36
4.6.本章小结	36
5.评价		37
5.1.对项目开发过程的体会	37
5.1.1.	过程总结	37
...
3.4.WEB服务端处理请求模块	90
3.4.1.	搜索请求处理测试用例	90
3.4.2.	注册登录请求处理测试用例	90
3.4.3.	评论或收藏请求处理测试用例	91
3.4.4.	数据爬取请求处理测试用例	92
4.测试结果分析	92
附录4 用户文档说明书	93
1.网站描述	93
2.使用说明	93
...
6)	系统的测试工作。
1.4.	报告组织与结构
第一部分：绪论。主要介绍购物网站的基于网络爬虫的商品评论导购网站的背景，以及相关领域的国内外研究状况。
第二部分：主要介绍基于网络爬虫的商品评论导购网站开发过程当中会用到的一些关键性的技术和知识，方便系统开发和实现部分的阅读与理解。 
第三部分：主要介绍基于网络爬虫的商品评论导购网站的开发和实现细节，是本报告的主体部分。 包括了系统的整体框架、模块定义、功能分配、系统各个模块的设计思想、重要功能的逻辑流程以及模块的接口设计等。就设计思路方面阐述了第二部分的知识理论如何在本系统中有效的利用。
第四部分：主要介绍基于网络爬虫的商品评论导购网站的测试。这部分针对系统的各个模块设计出了测试用例，并通过表格的形式反映系统各个功能的测试结果。
第五部分：主要介绍基于网络爬虫的商品评论导购网站开发工作的评价。
...
3.	系统设计与开发
3.1.	项目概况
本系统为windows环境下基于Node.js开发的一个基于网络爬虫系统的商品评论网站。整个系统由客户端和web服务端两个部分组成。整个系统用node.js为语言基础，express为技术框架、MySQL存储数据，用Superagent爬取真实网站的商品数据，用Cheerio过滤并获取数据，用Canvas对数据以图表的方式可视化。最后网站在商品页给用户提供一个基于评论数据直观化的评论图表，直观的反应出该商品的优劣。并在客户端以图表的形式展示该产品的主要评论信息，以供用户更加深入的进行判断。web服务端部分则需要支持整个网站的运营以及后台管理，数据爬起等。
3.2.	概要设计
3.2.1.	系统结构设计
本系统包括客户端和web服务端。客户端主要划分成3个模块：用户交互模块、信息展示模块和后台管理模块。web服务端主要用于处理客户端的请求，统一为一个模块：服务端处理请求模块。每个模块包含了相应的功能点，整个系统的结构如图3.1.1，网络模型如图3.1.2：
...
表3.2用户交互模块功能定义表
编号	功能点	功能点详细内容
USER_REG	注册	判断新用户信息是否符合规范，数据库写入新用户信息
...
USER_CRAW	商品管理	判断用户是否为管理员，爬取数据存入数据库
USER_ADMIN	用户管理	删除用户，增删管理员
...
表3.3信息展示模块功能定义表
序号	功能点			功能点详细内容
INFO_DEAL	根据URL跳转至商品信息展示页面	根据请求返回的数据展示商品信息页面
INFO_COMM	根据URL跳转至评论信息展示页面	根据请求返回的数据展示评论信息页面，评论图表
INFO_USER	根据URL跳转至用户信息展示页面	判断用户登录状态并根据请求返回的数据展示用户信息页面
INFO_BRAND	根据URL跳转至品牌信息展示页面	根据请求返回的数据展示商品信息页面
2)	性能要求
信息展示模块是系统的基础模块之一，为了呈现较好的用户体验，要求页面跳转流畅进行。商品信息准确无误，不会出现超时延迟。
3)	设计方法
信息展示模块的核心是Node请求路由的跳转，在客户端要做出请求失败，请求数据暂无状态的反馈页面：
...
表3.4后台管理模块功能定义表
序号	功能点	功能点详细内容
ADMIN_USER	用户管理	增删管理员，删除用户
ADMIN_CRAW	商品管理	爬取目标网站商品信息写入数据库
2)	性能要求
本模块需要爬取数据准确无误，管理信息修改准确无误，商品信息修改及时生效，在正确的前提上逐步提升性能。
3)	设计方法
本模块主要为后台管理员操作按钮爬取目标位网站数据，管理网站用户等。具体如下：
商品管理：台管理员操作按钮爬取目标位网站数据。
用户管理：对用户数据的增删查改。
4)	逻辑流程
	商品管理逻辑流程
...
图3.18商品管理逻辑流程图
	用户管理逻辑流程
管理员对普通用户进行删除或者设置为管理员操作，对其他管理员进行设为普通用户操作，模块发送请求至web服务端，web服务端操作数据后返回成功信息。具体流程如图3.19：
...
序号	功能点	功能点详细内容
SERV_SEARCH	响应客户端请求	从接收到的请求判断请求，有数据则提取数据，增删查改数据库并返回结果给客户端
SERV_CRAW	数据爬取	爬取目标网站数据
SERV_DATA	数据处理	过滤数据获取有效的数据
SERV_SAVE	数据存储	将有效的数据存入数据库
2)	性能要求
该模块为系统的web服务端部分，需要处理来自客户端的请求。基本要求是web服务端能识别来自客户端的各种请求并给出正确的回应。能够在可接受的时间范围内对客户端请求作出回应。
爬取到的数据正确的提取出有效数据并存入数据库。
3)	设计方法
web服务端的设计采用Node.js 事件驱动， 非阻塞I/O 模型得以轻量和高效。就具体功能来说，设计如下：
响应客户端请求：当接收到客户端的请求后，解析出URL，判断请求是否需要查询数据库，是则，调用数据库访问接口，传入sql语句进行查询，然后传回封装好的查询结果或数据处理结果，最后回传到客户端。不需要则直接返回结果给客户端。
数据爬取：用Superagent库的方法爬取目标网站的数据，返回一个结果集。
数据分析：用Cheerio库的方法对上一步返回的结果集进行数据过滤，提取有效的数据，对商品数据直接过滤，对评论数据进行分析并产出新的分析数据。
数据存储：用MySQL存储上一步提取到的有效的数据。数据库表格见数据库设计。
4)	逻辑流程
	响应客户端请求逻辑流程：
web服务端接收到客户端的请求后，解析出URL，判断请求是否需要查询数据库，是则调用数据库访问接口，传入sql语句进行查询，然后传回封装好的查询结果或数据处理结果，最后回传到客户端。不需要则直接返回结果给客户端。流程如图3.20：
 
图3.20响应客户端请求逻辑流程图
	数据爬取逻辑流程：
服务端接收到的数据爬取请求，便开始爬取目标网站数据，将结果进行封装成一个结果集。如图3.21：
 
图3.21数据爬取逻辑流程图
	数据分析逻辑流程：
服务端对数据爬取到的结果集进行分析过滤得到有效的数据。使用Cheerio的load方法对数据爬取到的结果进行数据转换，用基于正则表达式原理的方法对数据进行过滤，最终得到有效的数据。流程如图3.22：
 
图3.22数据分析逻辑流程图
	数据存储逻辑流程：
服务端将数据分析到的有效数据进存储到数据库。其中包含商品ID，商品购买链接，商品图片链接，商品名称，商品类型，商品价格等。评论数据信息是实时爬取实时分析实时展示的。流程如图3.22：
 
图3.23数据存储逻辑流程图
5)	接口设计
Route为web服务端主要模块，分发请求路由。Star为商品爬取模块、数据分析模块，Model提供数据库方法，Mysql用于连接数据库。
3.4.	数据库设计
3.4.1.	数据逻辑描述
图3.23描述了整个网站的数据流向关系。主要包括搜索查询、注册登录、品牌评论、品牌收藏、数据爬取、用户管理等数据传输：
 
图3.23网站数据流图
3.4.2.	数据库表格详细信息
图3.24是系统数据库表格一览，其中userinfo存储用户信息，dealall存储爬取的商品信息，dealcoll存储收藏商品的信息，comments存储评论内容相关信息，其他表格按分类存储的商品信息：
 
图3.24数据库表格一览
图3.25是存放用户信息的数据表，其中id为整形自增类型，nickname，email，password为字符串类型，保存用户的昵称，邮箱，密码信息，status为整形，保存判断用户是否为管理员的数据信息：
 
图3.25 user_info数据表

图3.26是存放商品信息的数据表，其中id为整形自增类型，href，picsrc，price，name，dealid分别表示商品的跳转链接，图片链接，价格，商品名称，商品独有的id。dealtype代表分类后商品所属的分类：
 
图3.26 dealall数据表

图3.27是存放用户收藏商品的数据，id为整形自增类型，dealid表示收藏的商品id，nickname表示收藏该商品的用户昵称：
 
图3.27 dealcoll数据表

图3.28是存放评论数据的表格，id为整形自增类型，type表示评论所属商品分类，username为评论者昵称，content表示用户评论内容，com_time表示评论时间：
...
4.1.	运行效果
运行环境为Chrome浏览器，版本号为49.0.2623.110。
4.1.1.	首页运行效果
图4.1为客户端首页部分运行效果，其中红框内为爬取的商品信息数据展示。其中商品的图片、名称、价格以及跳转至购买页的URL都是爬取到的信息：
 
图4.1 首页运行效果图
4.1.2.	评论蛛网图表运行效果
图4.2为客户端部分评论蛛网图表运行效果图，其中好评率、正品、质量不错、没有色差、穿着舒适、物流快等流向数据是对爬取到的数据进行分析，最终以可视化的图表进行展示。如图，蛛网图的每个角占得面积越大，表示商品在这一评论区的得分越高：
 
图4.2 客户端部分评论蛛网图表运行效果图
4.1.3.	评论柱状图表运行效果
图4.3为客户端部分评论柱状图表运行效果图，其中好评率、正品、质量不错、没有色差、穿着舒适、物流快等流向数据是对爬取到的数据进行分析，灰色代表标准分，鲜色代表实际分。最终以可视化的图表进行展示。如图，灰色代表商品各评论的满分标准，浅蓝色代表商品评论的实际得分：
 
图4.3 评论柱状图运行效果图

4.1.4.	商品详情页运行效果
图4.4为客户端商品详情页部分运行效果，其中红框中的数据为实时爬取的数据，点击立即购买跳转到商品网站进行购买：
 
图4.4 商品详情页运行效果图
4.1.5.	商品评论内容页运行效果
图4.5为客户端商品评论内容页部分运行效果，其中红框中的数据为实时爬取的数据，点击查看更多评论可连接到更多评论的页面：
...
评论页下滑查看评论图表	评论图表正常	评论图表正常	PASS
...
4.4.	后台管理模块测试
4.4.1.	商品管理能测试用例
这里对商品管理的各种状态和各种交互效果进行测试用例编写，具体如表4.8：
表4.8商品管理功能测试用例表
测试用例ID	Admin1
测试用例名称	数据爬取
前提条件	登录用户为管理员
...
小结
由于时间和商业的关系，该项目还存在一些欠缺。一些设想没有时间去实现，一些想法没有商业支持去实现。如：同步用户功能到商品目标网站、对爬取到的评论信息进行大数据分析、定制网站爬取、直接用本网站的用户名去购买等等。这些可以作为系统后续功能扩展的一个参考和指引。
...
1.	引言
1.1.	编写目的
电子商务是电子信息和互联网技术结合厂商或其他交易主体之间进行的信息商务活动，而网上购物正是这种活动的表现形式之一。网上购物影响我们的生活越来越深。然而现代主流的购物网站都具有十分丰富的界面和功能，但是还是有些不尽如人意的地方。因此，在借鉴其他主流导购网站已经十分完善的功能的基础上，开发出一款基于评论的导购网站具有一定的意义。
...
2.2.	web服务端功能描述
2.2.1.	web服务端处理请求模块
	响应客户端请求，接收客户端的数据处理请求，连接数据库进行数据操作并返回结果。
	数据爬取，接收数据爬取请求，连接网络爬取目标网站的数据并返回结果集。
	数据分析，将上一步得到的数据结果集进行分析过滤，得到有效数据并返回结果。
	数据存储，将上一步的有效数据存入数据库。
 
3.	数据模型
3.1.	数据逻辑描述
图3.1描述了整个网站的数据流向关系。主要包括搜索查询、注册登录、品牌评论、品牌收藏、数据爬取、用户管理等数据传输：
 
图3.1网站数据流图
3.2.	数据库表格详细信息
图3.2是系统数据库表格一览，其中userinfo存储用户信息，dealall存储爬取的商品信息，dealcoll存储收藏商品的信息，comments存储评论内容相关信息，其他表格按分类存储的商品信息：
 
图3.2数据库表格一览图
图3.3是存放用户信息的数据表，其中id为整形自增类型，nickname，email，password为字符串类型，保存用户的昵称，邮箱，密码信息，status为整形，保存判断用户是否为管理员的数据信息：
 
图3.3 user_info数据表

图3.4是存放商品信息的数据表，其中id为整形自增类型，href，picsrc，price，name，dealid分别表示商品的跳转链接，图片链接，价格，商品名称，商品独有的id。dealtype代表分类后商品所属的分类：
 
图3.4 dealall数据表

图3.5是存放用户收藏商品的数据，id为整形自增类型，dealid表示收藏的商品id，nickname表示收藏该商品的用户昵称：
 
图3.5 dealcoll数据表

图3.6是存放评论数据的表格，id为整形自增类型，type表示评论所属商品分类，username为评论者昵称，content表示用户评论内容，com_time表示评论时间：
...
表3.1数据具体表现表
功能	输入	处理	输出
搜索商品	搜索关键词	传送关键词并查询数据库	返回的查询结果
信息展示	网页路径	根据路径传送信息并查询数据库	返回的查询结果
注册登录	用户信息	解析信息并增、查数据库	web服务端反回更新数据库的结果
品牌评论	评论信息	解析信息并增、查数据库	web服务端反回更新数据库的结果
收藏商品	商品ID	传送ID并增、删、查数据库	web服务端反回更新数据库的结果
数据爬取	爬取的URL	解析URL并爬取数据存入数据库	web服务端返回存入数据库返回的结果
用户管理	用户信息	解析信息并增、删数据库	web服务端反回更新数据库的结果
信息修改	提交的内容	解析信息并改数据库	web服务端反回更新数据库的结果
 
4.	行为模型
行为模型用以描述行为与系统状态之间的关系。通过构建行为模型，描述在接收到用户各种行为时系统状态的影响和应该做出的反应，通常使用状态图来描述。
4.1.	客户端行为模型
4.1.1.	登录行为模型
登录行为主要是用户进入登陆页，输入符合规范的信息，点击登录，后台处理登录请求返回结果给客户端，继续等待用户操作，图4.1是对登录行为进行状态描述。
 
图4.1登录行为状态图
4.1.2.	注册行为模型
注册行为主要是用户进入注册页，输入符合规范的信息，点击注册，后台处理注册请求返回结果给客户端，继续等待用户操作，图4.2是对注册行为进行状态描述。
 
图4.2注册行为状态图
4.1.3.	评论行为模型
评论行为主要是用户进入品牌评论页，输入符合规范的评论内容，点击发表评论，后台处理请求返回结果给客户端，继续等待用户操作，图4.3是对评论行为进行状态描述。
 
图4.3评论行为状态图
4.1.4.	收藏行为模型
收藏行为主要是用户进入商品详情页，登录状态下点击收藏，后台处理收藏请求返回结果给客户端，继续等待用户操作，图4.4是对收藏行为进行状态描述。
 
图4.4收藏行为状态图
4.1.5.	信息修改行为模型
信息修改行为主要是用户进入信息修改页，输入符合规范的原始密码新密码，点击修改，后台处理修改请求返回结果给客户端，继续等待用户操作，图4.5是对信息修改行为进行状态描述。
 
图4.5信息修改行为状态图
4.1.6.	搜索行为模型
搜索行为主要是用户进入网站，输入搜索关键词，后台处理搜索请求返回结果给客户端，继续等待用户操作，图4.6是对搜行为块进行状态描述。
 
图4.6搜索行为状态图
4.1.7.	用户管理行为模型
用户管理行为主要是管理员进入后台管理，点击功能按钮触发请求，后台处理请求返回结果给客户端，继续等待管理员操作，图4.8是对用户管理行为进行状态描述。
 
图4.8用户管理行为状态图
4.1.8.	商品管理行为模型
管理员点击功能按钮触发事件，模块向web服务端发送请求，并开始对数据进行爬取、过滤，存入数据库，爬取成功则返回成功的信息，图4.9是对商品管理行为进行状态描述。
 
图4.9商品管理行为状态图
4.2.	服务端行为模型
4.2.1.	响应客户端请求行为模型
web服务端接收到客户端的请求后，解析出URL，判断请求是否需要查询数据库，是则调用数据库访问接口，传入sql语句进行查询，然后传回封装好的查询结果或数据处理结果，最后回传到客户端。不需要则直接返回结果给客户端。图4.10是响应客户端请求行为进行状态描述。
 
图4.10响应客户端请求行为状态图
4.2.2.	数据爬取行为模型
服务端接收到的数据爬取请求，便开始爬取目标网站数据，将结果进行封装成一个结果集。图4.11是数据爬取行为进行状态描述。
 
图4.11数据爬取行为状态图
4.2.3.	数据分析行为模型
服务端对数据爬取到的结果集进行分析过滤得到有效的数据。图4.12是数据分析行为进行状态描述。
 
图4.12数据分析行为状态图
4.2.4.	数据存储行为模型
服务端将数据分析到的有效数据进存储到数据库，具体数据格式见数据模型。图4.13是数据存储行为进行状态描述。
...
附录2 设计文档
1.	系统结构设计
本系统包括客户端和web服务端。客户端主要划分成3个模块：用户交互模块、信息展示模块和后台管理数据爬取模块。web服务端主要用于处理客户端的请求，统一为一个模块：客户端请求处理模块。每个模块包含了相应的功能点，整个系统的结构如图1.1.1，网络模型如图1.1.2：
...
接口：通过网络套接口传递数据。
2.4.	后台管理模块概要设计
功能：后台管理模块主要是有管理员身份的用户对网站的普通用户以及其他管理员的管理。数据爬取是有管理员身份的用户点击相应功能按钮触发事件，对目标网站进行数据爬取，并存入数据库，等待客户端请求数据。具体如图2.3：
...
功能：web服务端处理请求模块主要是处理来自客户端的不同请求。如图2.4：
 
图2.4 web服务端处理请求模块功能结构图
输入：来自客户端的请求。
输出：对客户端请求的反馈。
接口：通过网络套接口传递数据。
...
3.2.2.	功能
用户交互应该实现的功能和对功能的定义如表3.1所示。需要注意用户评论、收藏、购买等交互需要用户状态为已登录为前提。用户注册需要判断用户名是否已经存在，信息是否符合规范；用户登录需要判断密码是否符合，后台管理及数据爬取需要用户身份为管理员：
表3.1用户交互模块功能表
编号	功能点	功能点详细内容
...
图3.7点击购买逻辑流程图
3.2.6.	接口设计
系统4个模块的接口的设计都遵循Node的请求路由设计规范，通过use/get/post等方法获取链接，分发路由，而对于模块类的成员尽可能的设计为私有，保证了良好的封装性，user提供用户交互的接口，通过export向外提供访问
3.2.7.	注释设计
	加在模块首部的注释。
	加在函数处的注释。
	加载各变量的功能的注释。
...
3.3.2.	功能
主要包括商品信息展示、评论信息展示、用户信息展示、品牌信息展示，表3.2是对功能的定义:
表3.2信息展示模块功能表
序号	功能点			功能点详细内容
INFO_DEAL	根据URL跳转至商品信息展示页面	根据请求返回的数据展示商品信息页面
INFO_COMM	根据URL跳转至评论信息展示页面	根据请求返回的数据展示评论信息页面，评论图表
...
3.4.2.	功能
后台管理模块主要是提供管理员管理网站用户，爬取目标网站数据并写入本地，具体功能的定义如表3.3：
表3.3后台管理模块功能定义列表
序号	功能点	功能点详细内容
ADMIN_USER	用户管理	增删管理员，删除用户等
ADMIN_CRAW	商品管理	爬取目标网站商品信息写入数据库
3.4.3.	性能
本模块需要爬取数据准确无误，管理信息修改准确无误，商品信息修改及时生效，在正确的前提上逐步提升性能。
3.4.4.	设计方法
本模块主要为爬取数据，管理网站用户以及商品数据。具体如下：
商品管理：通过Node库Superagent对目标网站进行数据爬取，通过Cheerio对数据进行过滤并最终存入数据库。
用户管理：对用户数据的增删查改。
3.4.5.	流程逻辑
	商品管理逻辑流程
...
3.5.2.	功能
由于客户端要求实现用户注册登录、信息修改、网页跳转、商品搜索、品牌评论、收藏商品、点击购买、数据爬取、用户管理等功能，因此该模块要求完成搜索处理、数据处理、路由分发、结果反馈等功能。功能定义如表3.4：
表3.4 web服务端处理请求模块功能表
序号	功能点	功能点详细内容
SERV_SEARCH	响应客户端请求	从接收到的请求判断请求，有数据则提取数据，增删查改数据库并返回结果给客户端
SERV_CRAW	数据爬取	爬取目标网站数据
SERV_DATA	数据处理	过滤数据获取有效的数据
...
3.5.5.	流程逻辑
	响应客户端请求逻辑流程：
web服务端接收到客户端的请求后，解析出URL，判断请求是否需要查询数据库，是则调用数据库访问接口，传入sql语句进行查询，然后传回封装好的查询结果或数据处理结果，最后回传到客户端。不需要则直接返回结果给客户端。流程如图3.14：
 
图3.14搜索请求处理逻辑流程图
	数据爬取逻辑流程：
服务端接收到的数据爬取请求，便开始爬取目标网站数据，将结果进行封装成一个结果集。如图3.15：
 
图3.15数据爬取逻辑流程图
	数据分析逻辑流程：
分发不同请求至不同的控制器进行处理，并对处理的结果返回至相应的客户端。流程如图3.16： 
...
3.3.	后台管理模块
3.3.1.	商品管理功能测试用例
这里对商品管理功能的各种状态和各种交互效果进行测试用例编写,具体见表3.8：
表3.8商品管理测试用例表
测试用例ID	Admin1
测试用例名称	数据爬取
前提条件	登录用户为管理员
...
在测试过程中，遇到过一些小bug，不过都很快解决了这些小bug。最终测试结果与预期相同，说明网站设计符合用户需求。
...
图2.4左侧导航推荐
点击账号信息栏，填写信息点击功能按钮可以修改信息。如图2.5:：
...
图2.7商品评论页
点击进入登录注册页，输入用户名、密码点击功能按钮即可。如图2.8：
